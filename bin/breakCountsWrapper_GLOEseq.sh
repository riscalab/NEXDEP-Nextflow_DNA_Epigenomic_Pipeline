...
echo "bam	bed	Expected_Density	Observed_Breaks	Read_Norm_Breaks" > densityCalculations.plus.log
echo "bam	bed	Expected_Density	Observed_Breaks	Read_Norm_Breaks" > densityCalculations.minus.log
for peak in $@
	do 
		echo "iterating"
				...
				do
					if [[ $bam == *.bam ]]
						then
							...
							expectedDensity=$(Rscript /lustre/fs4/home/rjohnson/pipelines/hera_pipeline/bin/getUniformBreakDensity.R $peak hg19 2>/dev/null)
							echo $expectedDensity
							conda activate fastq2bam
							bash /lustre/fs4/home/rjohnson/pipelines/hera_pipeline/bin/getBreakDensityInPeaksV3_GLOEseq.sh $bam $peak
							observedBreaks=$(awk '{ total += $NF } END { print total }' ${bam##*/}.${peak##*/}.numBreaksInPeaks.bed)
							observedBreaksPlus=$(awk '{ total += $NF } END { print total }' ${bam##*/}.${peak##*/}.numBreaksInPeaks.plus.bed)
							observedBreaksMinus=$(awk '{ total += $NF } END { print total }' ${bam##*/}.${peak##*/}.numBreaksInPeaks.minus.bed)
							echo $observedBreaks
							observedDensity=$(awk '{ total += $NF } END { print total }' ${bam##*/}.${peak##*/}.numBreaksInPeaksNormalized.bed)
							observedDensityPlus=$(awk '{ total += $NF } END { print total }' ${bam##*/}.${peak##*/}.numBreaksInPeaksNormalized.plus.bed)
							observedDensityMinus=$(awk '{ total += $NF } END { print total }' ${bam##*/}.${peak##*/}.numBreaksInPeaksNormalized.minus.bed)
							...
							echo "$bam	$peak	$expectedDensity	$observedBreaksPlus	$observedDensityPlus" >> densityCalculations.plus.log
							echo "$bam	$peak	$expectedDensity	$observedBreaksMinus	$observedDensityMinus" >> densityCalculations.minus.log
					fi
				done
		fi
	done
