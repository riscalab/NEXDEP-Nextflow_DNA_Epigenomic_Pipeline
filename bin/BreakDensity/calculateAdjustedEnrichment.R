...
for(peak in 1:length(unique(enrichmentScores$bed))){
  ...
  for(i in 1:length(Cells)){
    adjustedEnrichment <- c(adjustedEnrichment, (Cells/PLCs[i]))
    for(j in 1:length(PLCs)){
      #print(CellsBams[i])
      #print(PLCsBams[j])
      #print(unique(enrichmentScores$bed)[peak])
      adjustedEnrichmentBams <- rbind(adjustedEnrichmentBams, c(CellsBams[i], PLCsBams[j], unique(enrichmentScores$bed)[peak]), stringsAsFactors = FALSE)
      #print(i)
      #print(j)
      #print(c(CellsBams[i], PLCsBams[j], unique(enrichmentScores$bed)[peak]))
    }
  }
}
...
adjustedEnrichmentBams <- cbind(adjustedEnrichmentBams, adjustedEnrichment)
colnames(adjustedEnrichmentBams) <- c("CellsBam", "PLCBam", "peak", "adjustedEnrichment")
write.table(adjustedEnrichmentBams, file='adjustedEnrichment.tsv', quote=FALSE, sep='\t', row.names = FALSE)

#print(adjustedEnrichmentBams$CellsBam)
adjustedEnrichmentBams$CellsBam <- str_remove(basename(adjustedEnrichmentBams$CellsBam),"_001.trim.st.all.blft.qft.rmdup.bam")
adjustedEnrichmentBams$PLCBam <- str_remove(basename(adjustedEnrichmentBams$PLCBam),"_001.trim.st.all.blft.qft.rmdup.bam")
adjustedEnrichmentBams$peak <- basename(adjustedEnrichmentBams$peak)
